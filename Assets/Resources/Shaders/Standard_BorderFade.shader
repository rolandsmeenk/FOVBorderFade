...
Shader "MixedRealityToolkit/Standard_BorderFade"
{
    Properties
    {
        // Main maps.
		...
		_FadeBorderWidth("Fade Border Width", Range(0.0, 500.0)) = 100
...
//#endif
            ...
            fixed4 frag(v2f i) : SV_Target
            {
				...
				float2 distanceInPixels = (0.5 - abs(pos - 0.5)) * _ScreenParams.xy;
				float mask = linearStep(0, _FadeBorderWidth, min(distanceInPixels.x, distanceInPixels.y));

				output *= mask;

				//return output + lerp(fixed4(1, 0, 0, 1), fixed4(0, 1, 0, 1), mask);
